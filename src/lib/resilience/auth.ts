/**
 * Vendored from cubiczan-resilience (typescript/src/auth.ts).
 * Relative import extensions stripped for this repo's module style.
 */
import {
 ...
 *
 * When the expected token is unset we return 503 (misconfigured) rather than
 * allowing the request; a mismatch or missing header is 401. This is the
 * generic predicate form — see {@link requireAuthResponse} for the
 * framework-agnostic Response helper.
 */
...
/**
 * Framework-agnostic helper. Returns a `Response` to send when the request is
 * rejected, or `null` when the caller is authorized.
 */
